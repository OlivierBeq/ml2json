...
def serialize_nearest_neighbors(model):
    return _base.serialize_model_generic(model, meta='nearest-neighbors')
...
def serialize_kernel_density(model):
    return _base.serialize_model_generic(model, meta='kernel-density')
...
def deserialize_kdtree(model_dict):
    return _base.deserialize_kdtree(model_dict)
...
if 'NNDescent' in __optionals__:
    ...
    def deserialize_nndescent(model_dict):
        return _base.deserialize_nndescent(model_dict)
